...
#def get_pie_chart(value):
def update_scattergraph(value,value1):
        if value == 'ALL':
#            filtered_df1 = spacex_df
            filtered_df1 = spacex_df[(spacex_df['Payload Mass (kg)']>=value1[0]) & (spacex_df['Payload Mass (kg)']<=value1[1])]
            fig1 = px.scatter(filtered_df1,x='Payload Mass (kg)',y='class',color='Booster Version Category',
            title='Correlation between Payload and Success for all Sites')
        else:
#            filtered_df1 = spacex_df[(spacex_df['Launch Site']==value)]
            filtered_df1 = spacex_df[(spacex_df['Payload Mass (kg)']>=value1[0]) & (spacex_df['Payload Mass (kg)']<=value1[1]) & (spacex_df['Launch Site']==value)]
            fig1 = px.scatter(filtered_df1,x='Payload Mass (kg)',y='class',color='Booster Version Category',
            title='Correlation between Payload and Success for site ' +  value)
#        return fig
